...
import com.mjhram.radioalarm.helpers.AudioFocusAwarePlayer;
import com.mjhram.radioalarm.helpers.AudioFocusHelper;
import com.mjhram.radioalarm.helpers.AudioFocusRequestCompat;
import com.mjhram.radioalarm.helpers.IcyDataSourceFactory;
import com.mjhram.radioalarm.helpers.LogHelper;
import com.mjhram.radioalarm.helpers.NotificationHelper;
import com.mjhram.radioalarm.helpers.PackageValidator;
import com.mjhram.radioalarm.helpers.PlayerCallback;
import com.mjhram.radioalarm.helpers.StationListProvider;
...
import static com.mjhram.radioalarm.helpers.StationListProvider.MEDIA_ID_EMPTY_ROOT;
import static com.mjhram.radioalarm.helpers.StationListProvider.MEDIA_ID_ROOT;
...
public final class PlayerService extends MediaBrowserServiceCompat implements TransistorKeys, AudioFocusAwarePlayer, Player.EventListener, MetadataOutput {
    ...
    public void onCreate() {
        ...
        audioManager = (AudioManager)
                this.getApplicationContext().getSystemService(Context.AUDIO_SERVICE);
        audioCurrentVol = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC);
        int audioMaxVol = audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
        int audioMinVol = audioManager.getStreamMinVolume(AudioManager.STREAM_MUSIC);

        int mid = (int) (1.0 * audioMinVol +.8*(audioMaxVol-audioMinVol));
        LogHelper.i(LOG_TAG, "Volume to:" + mid);
        LogHelper.i(LOG_TAG, "Volume Min:" + audioMinVol);
        LogHelper.i(LOG_TAG, "Volume Max:" + audioMaxVol);
        audioManager.setStreamVolume(AudioManager.STREAM_MUSIC,mid, AudioManager.FLAG_SHOW_UI);
        // set up variables
    ...
    private class InitializePlayerHelper extends AsyncTask<Void, Void, Integer> {
        ...
        protected void onPostExecute(Integer connectionType) {
            if (connectionType == CONNECTION_TYPE_ERROR) {
                ...
                LogHelper.i(LOG_TAG, "Volume back" + audioCurrentVol);

                //audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, audioCurrentVol, AudioManager.FLAG_SHOW_UI);
